...
def deposit_handler(player: Player, response: int, list_item: int, input_text: str, *args, **kwargs) -> bool:
    ...
    bank_account_id = try_pars_int(args[0])

    if bank_account_id is not None:
        with MAIN_SESSION() as session:
            bank_acc: BankAccount = session.query(BankAccount).filter(BankAccount.id == bank_account_id).first()
            bank_acc.balance += amount
            session.commit()

    else:
        with MAIN_SESSION() as session:
            bank_acc: BankAccount = session.query(BankAccount).filter(BankAccount.number == args[0]).first()
            bank_acc.balance += amount
            session.commit()
...
def list_player_bank_accounts(player: Player, *args) -> str:
    with MAIN_SESSION() as session:
        bank_accounts: list[BankAccount] = session.query(BankAccount).filter(BankAccount.owner_id == player.dbid).all()
...
def create_new_bank_account(player: Player, response: int, list_item: int, input_text: str, *args) -> None:
    ...
    with MAIN_SESSION() as session:
        ...
        bank_acc: BankAccount = BankAccount(number=number, password=password, balance=balance,
...
def check_bank_password(player: Player, response: int, list_item: int, input_text: str, *args, **kwargs) -> bool:
    if args:
        ...
        with (MAIN_SESSION() as session):
            bank_accounts: BankAccount = session.query(BankAccount).filter(BankAccount.id == bank_acc_id,
                                                                           BankAccount.password == input_text
                                                                           ).one_or_none()
